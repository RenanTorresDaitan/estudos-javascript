...
SkillShareServer.prototype.updated = function () {
  ...
  this.waiting = [];
  writeFile(talksFile, JSON.stringify(this.talks), (error) => {
    if (error) console.log(`Failed to write talks to file: ${error}`);
  });
};
...
function loadTalks() {
  let loaded;
  try {
    loaded = JSON.parse(readFileSync(talksFile, "utf-8"));
  } catch (error) {
    loaded = {};
  }
